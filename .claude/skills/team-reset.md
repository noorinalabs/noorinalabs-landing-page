...
Handle the team teardown/create lifecycle for the this project.
...
2. Manually edit the config file to remove stale members:
   ```bash
   # Read current config
   cat ~/.claude/teams/this repository/config.json
...
Call `TeamDelete` for team `this repository`. Report success or failure to user.

```
**Team deleted:** this repository
...
Read all roster files in `.claude/team/roster/` to build the new roster. Call `TeamCreate` for team `this repository`.

Report to user:

```
**Team created:** this repository
| Role | Name | Status |
|------|------|--------|
| Manager | Fatima Okonkwo | Active |
| ... | ... | ... |
...
```
Team `this repository` is ready. {N} members active.
...
```bash
# Remove stale config
rm ~/.claude/teams/this repository/config.json
