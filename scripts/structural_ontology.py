#!/usr/bin/env python3
"""Generate / staleness-check landing-page's structural ontology index (#155).

This is the noorinalabs-landing-page (consumer) side of the C×T2 distributed
...
* ``register-merge-driver`` -- register the union merge-driver in git config (the
  ...
  local state, so ``make setup-hooks`` / ``pre-commit install`` runs this).
  NOTE: the driver is invoked in PYTHON MODULE form
  (``python3 -m ontology_gen.merge_driver``), NOT as a bare script --
  ``merge_driver.py`` uses a package-relative import (``from .model``), so a
  plain ``python3 .../merge_driver.py`` invocation raises ImportError.
"""
...
REPO_NAME = "noorinalabs-landing-page"
